...
class Purchases extends MY_Controller {
    ...
    protected function owns($id)
    {
        // Fetch the current user's id
        $user_id = user('id');

        // Then fetch the purchase's owner's id
        $purchase_owner_id = $this->purchases_model->get_user_id($id);

        // Return true if they match
        if ($user_id == $purchase_owner_id) {
            return true;
        }
    ...
    {
        // Make sure the current user owns this purchase
        if (!$this->owns($id)) {
            show_404();
        }
    ...
    public function destroy()
    {
        ...
        // Make absolutely sure the user owns this purchase
        if ($this->owns($id)) {
